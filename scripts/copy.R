...
scrape_creek_data <- function(creek_node) {
  ...
  print(paste("Flow Volume: ", flow_volume))
  ...
  hydrologic_data <<- data.frame(flow_volume = flow_volume,stage = stage)
...
server <- function(input, output, session) {
  ...
  creek_data_reactive <- eventReactive(input$goButton, {
    ...
    # Scrape the data (flow volume, stage)
    data <- scrape_creek_data(creek_node)
    
    # Return the scraped data directly (not as a formatted string)
    return(data)
  })
  ...
  output$creekData <- renderText({
    ...
    if ("Water Conditions" %in% input$water_data) {
  ...
  output$weatherData <- renderText({
    ...
    if ("Weather" %in% input$weather_data) {
  ...
  output$safetyReport <- renderText({
    ...
    if ("Safety Report" %in% input$recommendation_type) {
      ...
      flow_volume <- hydrologic_data$flow_data
      stage <- hydrologic_data$stage_data
      
      # Ensure flow and stage are numeric, and check if they are NA or 0
      if (is.na(flow_volume) || is.na(stage) || flow_data == 0 || stage_data == 0) {
        selected_data <- "There is not enough data to run our safety model."
      } else {
        velocity_input <- flow_data / stage_data  # Compute velocity
